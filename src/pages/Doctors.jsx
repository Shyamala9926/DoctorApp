...
function Doctors() {
  ...
  useEffect(() => {
    if (speciality) {
      ...
      setFilteredDocs(doctors.filter(doc => doc.speciality.toLowerCase() === decodedSpeciality));
    } else {
      ...
      <div className="flex flex-col sm:flex-row items-start gap-5 mt-5">
        <div className="flex flex-col gap-4 text-sm text-gray-600">
          {["General physician", "Gynecologist", "Dermatologist", "Pediatrician", "Neurologist", "Gastroenterologist"].map(spec => (
            <p
              key={spec}
              onClick={() => navigate(speciality === spec ? '/doctors' : `/doctors/${encodeURIComponent(spec)}`)}
              className={`cursor-pointer border px-4 py-2 rounded-md transition-all ${speciality === spec ? "bg-indigo-100 text-black" : "border-gray-300"}`}
            >
              {spec}
            </p>
          ))}
        ...
        <div className="w-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          {filteredDocs.length > 0 ? (
            ...
            <p className="text-gray-500 text-center col-span-3">No doctors found for this specialty.</p>
          )}
        </div>
      </div>
    </div>
